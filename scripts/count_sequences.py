...
def main():
    ...
    for read in bam:
        if read.aligned:
            seq = read.read.seq.decode('utf-8')
            # find to how many loci it mapps
            mapped_loci = read.optional_field('XM') - 1
            if seq in seq_counts:
                seq_counts[seq]+= 1/mapped_loci
            else:
                seq_counts[seq] = 1/mapped_loci

    if options.verbose:
        sys.stdout.write(
            "Creating pandas dataframe from counts dictionary {}".format(
                os.linesep
            )
        )
    df = pd.DataFrame.from_dict(seq_counts, orient='index')
